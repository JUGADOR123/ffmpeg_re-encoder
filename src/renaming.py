...
class Rename:
    def __init__(
        ...
        self.base_folder=base_folder.absolute()
    ...
    def _rename(self,item:Path)->Path:
        ...
        if self.to_remove in item.name:
            try:
                new_item=item.rename(item.parent/item.name.replace(self.to_remove,self.to_add))
            except Exception as e:
                print(f"{RED}Failed to rename {BLUE}{str(item)!r}{RED}: {ORANGE}{e}{RESET}")
                self.failed.append((str(item), str(e)))
            else:
                print(f"{GREEN}Renamed {BLUE}{item.name!r}{GREEN} to {BLUE}{new_item.name!r}{RESET}")
        print(f"{ORANGE}Nothing renamed!{RESET}")
        return new_item or item

    def _process_folder(self,directory:Path):
        try:
            ...
            print(f"{RED}Failed to process {BLUE}{str(directory)!r}{RED}: {ORANGE}{e}{RESET}")
